...
{
    public partial class RiskBirimSkor : BaseChart
    {
        ...
        private Series databindchart(string serieName, List<RiskSkorEntity> data)
        {
           ...
           // serie.PostBackValue = string.Format("{0}:{1}", serie.Name, "#AXISLABEL");
            serie.Points.DataBind(data, "Skor", "SkorValue", "Label=SkorValue,LegendText=Skor");
            return serie;
        }
        ...
        protected override void DataBindChart(WidgetInstance instance)
        {
            ...
            var data = new RiskReportBusiness().RetreiveRiskPuanGroupByUnit(listingParams);
            ThisChart.Series.Clear();
            databindchart("Risk Birim Skor", data);
            SetPointColorsByValue(ThisChart.Series["Risk Birim Skor"]);
