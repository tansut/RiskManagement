...
{
    public partial class RiskSayi : BaseChart
    {
        ...
        protected void ThisChart_Click(object sender, ImageMapEventArgs e)
        {
            if (!string.IsNullOrWhiteSpace(e.PostBackValue))
            {
                ...
                UnitBusiness cb = new UnitBusiness();
                
                RiskFilter f = new RiskFilter();
                string[] value = e.PostBackValue.Split(":".ToCharArray(), StringSplitOptions.None);
                f.ArtikSkors.AddRange(rmb.GetGrupDegerIDs());
                string script = string.Format("openPageAsTab('{0}?filter={1}&Birims={2}','{3}');", Page.ResolveUrl("~/Pages/Risk/default.aspx"), HttpUtility.UrlEncode(f.ToString()), cb.GetBirimByIlName(value[1]), "Risk Değerlendirme");
        ...
        protected override void DataBindChart(WidgetInstance instance)
        {
            ...
            var data = new RiskReportBusiness().RetreiveRiskSayiGroupByUnit(listingParams);
