...
namespace Kalitte.WidgetLibrary.DashboardEditor
{
    public partial class Editor : System.Web.UI.UserControl, IWidgetEditor
    {
        ...
        private void BindDashboards(object key, int selectedIndex)
        {
            ...
            {
                list = (DashboardFramework.GetDashboards(Thread.CurrentPrincipal.Identity.Name).OrderBy(p => p.Title).OrderBy(p => p.DisplayOrder).OrderBy(p => p.Group).OrderBy(p => p.GroupDisplayOrder)).ToList();
                var merkez = DashboardFramework.GetDashboards().SingleOrDefault(p => p.UserTag == "MERKEZANASAYFA");
                var birim = DashboardFramework.GetDashboards().SingleOrDefault(p => p.UserTag == "BIRIMANASAYFA");
                if (merkez != null)
                    list.Add(merkez);
                if (birim != null)
                    list.Add(birim);
        ...
        private void GetInstanceFromUser(DashboardInstance instance, bool fillLayout)
        {
            ...
            var shareType = (DashboardShareType)Enum.Parse(typeof(DashboardShareType), ctlDashboardSharing.SelectedValue);
            if (!UserBusiness.UserHasGlobalRights())
                shareType = DashboardShareType.None;
        ...
        protected void ctlDashboardUpdate_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                ...
                if (instanceKey != null)
                {
                    ...
                    CheckIfUserHasRightToEdit(instance);
        ...
        private void CheckIfUserHasRightToEdit(DashboardInstance instance)
        {
            if (instance.Username != Thread.CurrentPrincipal.Identity.Name)
            {
                bool specialDashboards = instance.UserTag == "MERKEZANASAYFA" || instance.UserTag == "BIRIMANASAYFA";
                bool userGlobal = UserBusiness.UserHasGlobalRights();
                if (userGlobal)
                {
                    if (!specialDashboards)
                        throw new SecurityException("Global yetkili olmanıza rağmen işlem yetkiniz bulunmamaktadır.");
                }
                else throw new SecurityException("İşlem yetkiniz bulunmamaktadır.");
            }
        }
            ...
            if (Page.IsValid)
            {
                ...
                if (ColumnsGrid.SelectedDataKey != null && instanceKey != null && rowInstanceKey != null)
                {
                    ...
                    if (col != row.Columns[row.Columns.Count - 1])
                        DoError(AppResource.YouCanOnlyDeleteLastColum);
                    else if (row.Columns.Count == 1)
                        DoError(AppResource.CannotDeleteDefaultColum);
                    else
                    {
