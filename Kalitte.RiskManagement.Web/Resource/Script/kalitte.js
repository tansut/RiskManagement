...
var renderMatrixCell = function (val, meta, record) {
    var template = '<div style="background-color:{0};width:{1}px;height:{2}px;" class="riskMatrixCell"><h2>{3}</h2><span>{4}</span></div>';
    var rec = record.data[this.dataIndex];
    var res = String.format(template, rec.Color, this.grid.cellSize, this.grid.cellSize, rec.Score, rec.Display);
    return res;
}
...
function openPageAsTab(url, title) {
    if (window.top == window)
        openTab(Ext.getCmp("ExampleTabs"), url, title);
    else openTab(parent.Ext.getCmp('ExampleTabs'), url, title);
}
...
function changeTheme(item) {
    ...
    ExampleTabs.items.each(function (p) {
...
function getGridRowIndex(grid) {
    if (grid.getSelectionModel().hasSelection()) {
        return grid.store.indexOf( grid.getSelectionModel().selections.items[0]);
    } else return null;
}
